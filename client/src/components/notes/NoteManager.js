...
const Notes = props => {
  return (
    <List.Item
      ...
      onClick={() => {
        props.onNoteSelect(props.note._id);
      }}
    >
      <List.Content>
        <Grid columns={2} stackable>
          <Grid.Row verticalAlign="middle">
            <Grid.Column>
              <List.Header as="a">{props.note.title}</List.Header>
            </Grid.Column>
            <Grid.Column>
              <List.Header>
                <NoteOptions
...
class NoteManager extends React.Component {
  ...
  createNewNotebook(name) {
    ...
    NoteService.addNotebook(this.props.currentUserObj.user_email, name)
      .then(rsp => {
  ...
  notebookUpdated = name => {
    NoteService.listNotes(this.props.currentUserObj.user_email, name)
      .then(notes => {
        this.setState({ notes });
        return;
      })
  ...
  selectNote(id) {
    ...
    var note = this.state.notes.find(note => {
      return note._id === id;
    });
    ...
    if (this.state.notes.length === 0) {
      return (
        <Segment placeholder>
          ...
          <Button primary onClick={this.createNewNote}>
            Create First Note
          </Button>
        </Segment>
      );
      ...
      return (
        <div className="ui container">
          <Grid container stretched columns={2}>
            ...
            <Grid.Column>{this.renderNoteEditor()}</Grid.Column>
          </Grid>
        </div>
      );
    }
              ...
              <Container textAlign="right">
                <Grid columns={4} stackable textAlign="right">
                  <Grid.Row verticalAlign="middle">
                    ...
                    <Grid.Column width={2}>
                      <Dropdown
                        placeholder={this.state.currentNotebook}
                        selection
                      ...
                      {this.state.currentNotebook === "Create" && (
                        <NotebookCreate
                          createNewNotebook={value =>
                            this.createNewNotebook(value)
                          }
                          setCurrentNotebook={() =>
                            this.setDefaultCurrentNotebook()
                          }
                        />
                      )}
                    </Grid.Column>
